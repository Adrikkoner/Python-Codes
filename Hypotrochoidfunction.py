...
def func1(theta,R,r,d):
     x=((R-r)*np.cos(theta))+ d*np.cos(theta)*(((R-r)*theta)/r)  #the parametric equations for the hyprotrochoids
     return x
def func2(theta,R,r,d):
     y=((R-r)*np.sin(theta))- d*np.sin(theta)*(((R-r)*theta)/r)  #the parametric equations for the hyprotrochoids
     return y    
