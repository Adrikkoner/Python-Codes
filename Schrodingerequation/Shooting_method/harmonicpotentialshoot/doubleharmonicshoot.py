...
x = np.linspace(-b, b, N)    # x-axis                   # spring constant                      # mass of the body             # classical HO frequency -
...
def main():    
    ...
    en = np.linspace(0, lamda*(C*L**4 - L**2), 1000)   # vector of energies where we look for the stable states
    ...
    for i in range(4):                                                 # For each of 1st 4 allowed energies
        ...
        plt.plot(x, psi_normal**2, label="E = %.2f" %E_zeroes[i])      # and plot it scaled for comparison    
    plt.legend()
